...
# Execute a SELECT query for Shipments
query_shipments = "SELECT * FROM Shipments"
cursor.execute(query_shipments)
...
for row in shipment_data:
    shipment = {
        ...
        'Status': row[4],  # Adjusted to match the column name in the table
        'ArrivalDate': row[5],
    }
...
# shipment_idddd = shipments['ShipmentID']
...
# while True:
    ...
    # for qwe in range(fake.random_int(min=4, max=7)):  # Adjust the number of records as needed
        ...
        for _ in range(fake.random_int(min=9, max=15)):
            ...
            quantity_received = fake.random_int(min=70, max=90)  # Adjust as needed
            ...
            query = f"INSERT INTO ShipmentDetails (ShipmentID, ProductID, QuantityReceived, CostPerUnit) " \
                    f"VALUES ({shipment_id}, {product_id}, {quantity_received}, {cost_per_unit})"
